...
/**
 * A class that represents a <code>let</code> or <code>where </code> construct
 * in Jaskell
 * 
 * This class defines a local environment which can have its own definitions
 * 
 * @author bailly
...
public class Let extends ExpressionBase implements Namespace {
	...
	public void bind(String name, Object expr) {
		...
		if (ex == null)
			...
			throw new CompilerException(
				"Name " + name + " already bound in this let-construct");
	}
  ...
  public Namespace getEnclosing() {
    Object ex = parent;
    while(!(ex instanceof Namespace)) ex=((Expression)ex).getParent();
    return (Namespace)ex;
  }
