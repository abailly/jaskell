...
/**
 * this class holds all declarations from a module
 * 
 * A Module is a namespace which can contains Jaskell-Core declarations and
 * other modules. There exists one default module named Main which is created
 * whether or not a real module named Main appears in the source code.
 * <p>
 * Modules are named hierarchically like Java packages with '.' as separator in
 * identifiers.
 * 
 * @author bailly
...
public class Module implements Expression, Namespace {
   ...
   * <p>
   * The imported namespace is used to resolved unqualified symbols if lookup in
   * current module failed.
   * 
   * @param imp
   *          a module
   */
   ...
   * <p>
   * Symbols prefixed with the given name will be resolved against this module.
   * 
   * @param name
   * @param imp
   */
  ...
  /**
   * Adds a module named name to this module. If name already exists as an used
   * identifier in this Module, throws an IllegalArgumentException
   * 
   * @param name
   *          name of module to add
   * @param module
   *          Module object to add. May not be null
   * @exception IllegalArgumentException
   *              if name is already used in this module
   */
  ...
  /**
   * Lookup object in module by name
   * 
   * This method retrieves an object stored in this module by its name. The name
   * must be an unqualified name and is looked for in this module
   * 
   * @param name
   *          an unqualified name to look for
  ...
  /**
   * Lookup object in module and parent module by name
   * 
   * This method is like lookup(String) but delegates lookup to parent module if
   * name is not defined in current module
   * 
   * @param name
   ...
   * 
   * Resolve a name with respect to imported modules
   * 
   * @param name
   * @return Expression
   */
  ...
  /**
   * Method resolveTypeImport.
   * 
   * Resolve a type name with respect to imported modules
   * 
   * @param name
   * @return Expression
   */
   ...
   * 
   * @param name
   *          name of symbol to bind
   * @param e
   *          an Expression to bind to a symbol
   */
  ...
  /**
   * Binds value to name in this module. This method does not complains if name
   * is already bound : it justs discards the old definition
   * 
   * @param name
   *          name of symbol
   * @param value
   ...
   * 
   * @param type
   *          an object of a subclass of Type or null
   */
  ...
  public void addTypeDefinition(Definition def) {
    String unqname = def.getName()
        .substring(def.getName().lastIndexOf('.') + 1);
