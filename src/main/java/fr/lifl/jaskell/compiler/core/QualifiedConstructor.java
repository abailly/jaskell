 ...
 * 
 * @author bailly
 * @version $Id: QualifiedConstructor.java 1154 2005-11-24 21:43:37Z nono $
 *  */
public class QualifiedConstructor extends Constructor {
	 ...
	 * @param type
	 */
	public QualifiedConstructor(String name) {
		super(name);
	}
	 ...
	 */
	public QualifiedConstructor(String name, Type type) {
		super(name, type);
	}
	...
	public Expression lookup(String vname) {
		...
		while (it.hasNext()) {
			...
			if (mod != null)
				...
				mod = (Module) parent.lookup(mname);
		}
	...
	public Object clone() {
		QualifiedConstructor qv =
			new QualifiedConstructor(getName(), getType());
		Iterator it = path.iterator();
		while (it.hasNext())
			qv.addPathElement((String) it.next());
