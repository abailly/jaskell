...
public class PrimitiveType extends TypeConstructor implements Primitives {
	...
	/**
	 * An iterator running over a FunctionType
	 * 
	 * This iterator is used to iterate over domains and ranges of Function types
	 * 
	 * @author bailly
	 * @version $Id: PrimitiveType.java 1154 2005-11-24 21:43:37Z nono $
	 */
		...
		public Object next() {
			if (!checkFunction(curFunction))
				throw new NoSuchElementException(
					"No more function types in " + function);
	 ...
	 * 
	 * @param type a type 
	 * @return an instance of Iterator for traversing components of the
	 * function or null if type is not afunction 
	 */
	...
	public static boolean checkFunction(Type ft) {
		if (!ft.getConstructor().equals(Primitives.FUNCTION)
			|| !ft.getKind().equals(SimpleKind.K))
			return false;
		return true;
	}
	...
	public PrimitiveType(
		...
		Class clas,
		Kind kind,
		TypeApplicationFormat format,
			TypeComparator compar) {
		super(name, kind);
		this.javaClass = clas;
		if(format != null) setApplyFormatter(format);
	...
	public boolean equals(Object obj) {
		return super.equals(obj)
			&& ((PrimitiveType) obj).javaClass.equals(javaClass);
	}
	...
	/**
	 * A method to create simply function types
	 * 
	 * This factory method is used to create function types from
	 * an domain and range types
	 * 
	...
	public static Type makeFunction(Type domain, Type range) {
		return TypeFactory.makeApplication(
			TypeFactory.makeApplication(FUNCTION, domain),
			range);
	}
