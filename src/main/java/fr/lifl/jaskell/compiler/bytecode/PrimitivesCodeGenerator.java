...
class StaticMethodPrimitiveCode implements PrimitiveCode {
	...
	public Instruction emit(ClassFile cf) {
		short mref =
			MethodRefData.create(cf.getConstantPool(), clname, mname, msig);
...
public class PrimitivesCodeGenerator {
	...
	static {
		primitives.put(
			...
			new AbsolutePrimitiveCode(new Sequence(null)._iadd()));
		primitives.put(
			Primitives.MUL_INT_INT,
			new AbsolutePrimitiveCode(new Sequence(null)._imul()));
		primitives.put(
			Primitives.SUB_INT_INT,
			new AbsolutePrimitiveCode(new Sequence(null)._isub()));
		primitives.put(
				Primitives.EQ_INT_INT,
				new AbsolutePrimitiveCode(
					new Sequence(null)._if_icmpeq(7)._iconst_0()._goto(4)._iconst_1()));
		primitives.put(
				Primitives.LE_INT_INT,
				new AbsolutePrimitiveCode(
					new Sequence(null)._if_icmple(7)._iconst_0()._goto(4)._iconst_1()));
		primitives.put(
				Primitives.GE_INT_INT,
				new AbsolutePrimitiveCode(
					new Sequence(null)._if_icmpge(7)._iconst_0()._goto(4)._iconst_1()));
		primitives.put(
				Primitives.LT_INT_INT,
				new AbsolutePrimitiveCode(
					new Sequence(null)._if_icmplt(7)._iconst_0()._goto(4)._iconst_1()));
		primitives.put(
				Primitives.GT_INT_INT,
				new AbsolutePrimitiveCode(
					new Sequence(null)._if_icmpgt(7)._iconst_0()._goto(4)._iconst_1()));
		primitives.put(
			Primitives.MAX_INT_INT,
			new AbsolutePrimitiveCode(
				new Sequence(null)._dup2()._if_icmpgt(4)._swap()._pop()));
		primitives.put(
			Primitives.NEG_INT,
			new AbsolutePrimitiveCode(new Sequence(null)._ineg()));
		primitives.put(
			Primitives.XOR,
			new AbsolutePrimitiveCode(new Sequence(null)._ixor()));

		primitives.put(
				Primitives.ADD_FLOAT_FLOAT,
				new AbsolutePrimitiveCode(new Sequence(null)._fadd()));

		primitives.put(
				Primitives.EQ_FLOAT_FLOAT,
				new AbsolutePrimitiveCode( /* perform xor on result of fp commparison */
					new Sequence(null)._fcmpg()._ixor()));

		primitives.put(
				Primitives.GT_FLOAT_FLOAT,
				new AbsolutePrimitiveCode( 
					new Sequence(null)._fcmpg()));
		primitives.put(
				Primitives.LT_FLOAT_FLOAT,
				new AbsolutePrimitiveCode( 
					new Sequence(null)._swap()._fcmpg()));

		primitives.put(
				Primitives.GE_FLOAT_FLOAT,
				new AbsolutePrimitiveCode( 
					new Sequence(null)._fcmpg()._iconst_m1()._if_icmpeq(7)._iconst_1()._goto(4)._iconst_0()));
		primitives.put(
				Primitives.LE_FLOAT_FLOAT,
				new AbsolutePrimitiveCode( 
						new Sequence(null)._fcmpg()._iconst_1()._if_icmpeq(7)._iconst_1()._goto(4)._iconst_0()));

		primitives.put(
			Primitives.NOT,
			new AbsolutePrimitiveCode(new Sequence(null)._ifeq(7)._iconst_1()._goto(4)._iconst_0()));
		...
		RelativePrimitiveCode error = new RelativePrimitiveCode() {
			public Instruction emit(ClassFile cf) {
				...
				short cref =
					ClassData.create(
						cf.getConstantPool(),
						"fr/lifl/jaskell/runtime/types/JError");
				short mref =
	 ...
	 * Method getCode.
	 * 
	 * Returns code sequence associated with given primitive function. The function
	 * must have been registered before using <code>registerPrimitiveCode()</code>.
	 * 
	 * @param pf
	...
	/**
	 * This method is called during the parsing process to register
	 * Native function definitions implemented as static methods in a Java class.
	 * This method handles the registration process so that the right function is
	...
	public static void registerStaticPrimitive(PrimitiveFunction pf) {
		...
		sig.append(')').append(
			TypeHelper.getInternalName(args[args.length - 1]));
		primitives.put(
			pf,
			new StaticMethodPrimitiveCode(clname, fname, sig.toString()));
	}

	/**
	 * This method registers a PrimitiveFunction object as generating bytecode
	 * which is inserted as is during code generation process.
	 * 
	 * @param pf
	 * @param inst
	 */
	public static void registerBytecodePrimitive(
