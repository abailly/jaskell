...
import fr.norsys.klass.bytes.DoubleData;
import fr.norsys.klass.bytes.FieldRefData;
import fr.norsys.klass.bytes.FloatData;
import fr.norsys.klass.bytes.Instruction;
import fr.norsys.klass.bytes.IntData;
...
public class CodeGenerator extends BytecodeGenerator {
	...
	/**
	 * This constructor is used to generate code in a context different
	 * from a Module which is the standard case.
	 * 
	...
	/**
	 * An abstraction compiles to a new Class in an enclosing context
	 * and returns a code sequence corresponding to instanciation of 
	 * this class. If the <code>innerClass</code> flag is set,
	 * 
	...
	public Object visit(Abstraction a) {
		...
		String fullname =
			BytecodeGenerator.encodeName2Java(
				ns.getName() + "/" + functionName);
		nsname = fullname;
		...
		if (classFile != null) {
			...
			short lambdaindex =
				ClassData.create(classFile.getConstantPool(), fullname);
			short ctorindex =
	...
	private MethodFileInfo makeLambdaCtor(ClassFile lambda, int argCount) {
		...
		seq
			._aload_0()
			.add(TypedInst.constInt(lambda, argCount))
			._invokespecial(ctor, lambda)
			._return();
		code.add(seq);
	...
	/**
	 * Method makeUnboxedEvalMethod.
	 * 
	 * The unboxed evaluation method assumes all strict arguments are already
	 * evaluated on stack and all others are unevaluated. 
	 * 
	 * @param lambda
	 * @param a
	 * @param range
	 * @return FieldFileInfo
	 */
	private MethodFileInfo makeUnboxedEvalMethod(
		ClassFile lambda,
		Abstraction a,
		Type range) {
		MethodFileInfo mfi = new MethodFileInfo(lambda);
		 ...
		 */
		CodeAttribute code =
			new CodeAttribute(lambda, (short) (1 + a.getCount()));
	...
	private String encodeType2Signature(Abstraction a, Type range) {
		...
		while (it.hasNext()) {
			...
			if (a.isStrict(i++))
				sig.append(
					TypeHelper.getInternalName(
						BytecodeGenerator.encodeType2Class(t)));
			else
				sig.append("Lfr/lifl/jaskell/runtime/types/JObject;");
		}
		sig.append(')').append(
			TypeHelper.getInternalName(
				BytecodeGenerator.encodeType2Class(ta.getRange())));
		return sig.toString();
	}

	/**
	 * Construct JObject eval() override for closure. The constructed method
	 * checks the number of currently applied args. If this number is equal to expected
	 * number of arguments, the typed eval method is called else the partially applied closure
	 * is returned
	 */
	private MethodFileInfo makeEval0Method(
		ClassFile lambda,
		Abstraction a,
		Type type) {
		...
		while (typeit.hasNext()) {
			...
			Class d =
				(Class) ((TypeApplication) t.getDomain()).getRange().visit(tv);
			seq
				._aload_0()
				._getfield(argsfield, lambda)
				.add(TypedInst.constInt(lambda, i))
				._aaload();
			/* eval and unbox argument if it is strict */
			if (a.isStrict(i))
				seq._invokeinterface(ieval, 1, lambda).add(
					Type2Class.unbox(lambda, d));
			i++;
		}
		// call real eval
		short evref =
			MethodRefData.create(
				lambda.getConstantPool(),
				lambda.getClassFileInfo().getName(),
				"eval",
				encodeType2Signature(a, t.getRange()));
	...
	/** returns the name of eval function to use according to
	    type */
	...
	/**
	 * Reindexes the bindings in Alternative a to take into
	 * account parameters for Closure
	 * 
	 * @param a
	 * @param closure
	 */
		...
		try {
			...
			if (!strictness) {
				if (lb.isStrict())
					...
					seq.add(Type2Class.unbox(classFile, cls)).add(
						TypedInst.store(cls,classFile, index));
				else
				...
				if (lb.isStrict())
					...
					seq.add(Type2Class.box(classFile, cls)).add(
						TypedInst.store(JObject.class,classFile, index));
			}
			return seq;
		} catch (Exception ex) {
			throw new IllegalArgumentException(
				"Cannot compute store instruction for variable " + lb);
		}
	...
	public Object visit(Application a) {
		...
		try {
			...
			throw new CompilerException(
				"Type of first member of application ["
					+ fun
					+ "] is not a function "
					+ funtype);
		}
		...
		if (fun instanceof Variable) { /* either variable or Constructor */
			...
			log.finer(
				"Making strict complete application for function "
					+ v
					+ "("
					+ abs
					+ ")");
	...
	/**
	 * Finish generating code for an application occuring in strict context
	 * 
	 * This code generate invocation to unboxed evaluation method for given 
	 * function assuming the closure object and arguments are on the stack. The
	 * main difference between this and non strict application is the 
	 * eval code which is added at the end of sequence. 
	 * 
	 * @param a
	 * @param abs
	 * @return Instruction
	 */
	private Instruction makeStrictCompleteApplication(
		Application a,
		Binder abs,
		Type abstype) {
		Iterator it = a.getArgs().iterator();
		...
		while (it.hasNext()) {
			Expression arg = (Expression) it.next();
			Type expect =
				((TypeApplication) ((TypeApplication) typeit.next())
					.getDomain())
					.getRange();
			if (abs.isStrict(i++)) {
			...
			if (strictness) {
				...
				if ((!acls.isPrimitive()) && !acls.equals(JObject.class)) {
					short cr =
						ClassData.create(
							classFile.getConstantPool(),
							TypeHelper.getExternalName(acls).replace('.', '/'));
					seq._checkcast(cr);
				}
			}
			popStrict();
		...
		if (abs instanceof PrimitiveFunction) {
			...
			iref =
				MethodRefData.create(
					classFile.getConstantPool(),
					encodeName2Java(clname),
					"eval",
					encodeType2Signature(ab, funtype));
			seq._invokestatic(iref, classFile);
	...
	/**
	 * Method makeNonStrictCompleteApplication.
	 * 
	 * Assumes the fun Instruction has done all application and evaluated functional member
	 * @param a
	 * @param fun
	 * @param funtype
	 * @return Instruction
	 */
	private Instruction makeStrictCompleteApplicationSpine(
		Application a,
		Instruction fun,
		Type funtype) {
		Iterator it = a.getArgs().iterator();
	...
	private Instruction makePartialApplication(
		Application a,
		Instruction fun,
		Type funtype) {
		Iterator it = a.getArgs().iterator();
		...
		short funr =
			ClassData.create(
				classFile.getConstantPool(),
				"fr/lifl/jaskell/runtime/types/JFunction");
		short funinit =
			InterfaceMethodData.create(
				classFile.getConstantPool(),
				"fr/lifl/jaskell/runtime/types/JFunction",
				"init",
				"()Lfr/lifl/jaskell/runtime/types/JFunction;");
		...
		short apply =
			InterfaceMethodData.create(
				...
				"fr/lifl/jaskell/runtime/types/JFunction",
				"apply",
				"(Lfr/lifl/jaskell/runtime/types/JObject;)Lfr/lifl/jaskell/runtime/types/JObject;");
		while (it.hasNext()) {
			Expression arg = (Expression) it.next();
	...
	private Instruction makeStaticConsCall(ConstructorDefinition def) {
		...
		String clname =
			BytecodeGenerator.encodeName2Java(
				((Class) ddef.visit(tv)).getName().replace('.', '/'));
		String mtname =
			BytecodeGenerator.encodeName2Java(
				def.getName().substring(def.getName().lastIndexOf('.') + 1));
		/* encode signature */
		...
		String factsig =
			initsig.append(")L").append(clname).append(';').toString();
	...
	public Object visit(Module a) {
		...
		String clname =
			encodeName2Java(a.getName().replace('.', '/') + "$Module");
			...
			try {
				...
				if (functionName.equals("main")
					...
					if (code == null) // submodules
						continue;
					classFile.add(
						makeMethod(functionName, code, def.getType()));
				}
	 ...
	 */
	private MethodFileInfo makeMethod(
		String name,
		Instruction code,
		Type type) {
	...
	/**
	 * Method generate main method in Main ns. The generated class
	 * can then be directly executed by java VM.<p>
	 * The main function generated is the usual main(String[]) from java
	 * and just invokes argument less main and evals it.
	 * 
	 * @param name
	 * @param def
	 * @return MethodFileInfo
	...
	public Object visit(StringLiteral a) {
		// store string in constant pool
		short idx =
			StringData.create(classFile.getConstantPool(), a.getString());
		// generate code
		Instruction inst = null;
		if (idx < 256)
			inst = new OneArgInst(Opcodes.opc_ldc,classFile, (byte) idx);
	...
	public Object visit(Variable a) {
		...
		if (ns != null) {
			...
			String tstr =
				"()"
					+ TypeHelper.getInternalName(
						BytecodeGenerator.encodeType2Class(def.getType()));
			/* 
	...
	/**
	 * Patterns are visited only during alternative code generation
	 * We generate a code for testing if top of stack matches given pattern
	 * and store variables
	 * 
	...
	public Object visit(ConstructorPattern a) {
		try {
			...
			short index =
				ClassData.create(
					classFile.getConstantPool(),
					TypeHelper.getInternalName(cls));
			Sequence seq = new Sequence(classFile);
			...
			while (it.hasNext()) {
				...
				short mref =
					FieldRefData.create(
						...
						TypeHelper.getInternalName(cls),
						"_" + i,
						TypeHelper.getInternalName(fcls));
	...
	public Object visit(QualifiedVariable a) {
		...
		throw new CompilerException(
			"Unable to find module needed for variable " + a.getName());
	}

	/**
	 * Method makePrimitiveClosure.
	 * 
	 * Create a closure object which encapsulates call to a primitive method. 
	 * 
	 * @param def
	 * @return Object
	 */
	private Object makePrimitiveClosure(Expression def, QualifiedVariable a) {
		...
		CodeAttribute code =
			new CodeAttribute(
				lambda,
				(short) (1 + TypeHelper.countArguments(sig)));
		...
		short primindex =
			MethodRefData.create(
				lambda.getConstantPool(),
				BytecodeGenerator.encodeName2Java(modname),
				fname,
				sig);
	...
	public Object visit(Conditional conditional) {
		...
		if (!t.equals(Primitives.BOOL))
			throw new CompilerException(
				"Illegal conditional expression, type is " + t);
		/* 
	...
	/**
	 * Method makeNonStrictCondition.
	 * 
	 * Nonstrict conditional boxes the instructions through call to 
	 * primitive function ifThenElse
	 * 
	 * @param inst
	 * @param itrue
	 * @param ifalse
	 * @return Object
	 */
	private Object makeNonStrictCondition(
		Instruction inst,
		Instruction itrue,
		Instruction ifalse) {
		short mref =
	...
	/**
	 * Method makeStrictCondition.
	 * 
	 * We generate a strict conditional by generating direct comparison between
	 * result of condition expression and 0 and juming to code
	 * 
	 * @param icond
	 * @param itrue
	 * @param ifalse
	 * @return Object
	 */
	private Object makeStrictCondition(
		Instruction icond,
		Instruction itrue,
		Instruction ifalse) {
		Sequence seq = new Sequence(classFile);
		seq.add(icond)._ifeq(itrue.size() + 6).add(itrue)._goto(
			ifalse.size() + 3).add(
			ifalse);
		return seq;
	}
