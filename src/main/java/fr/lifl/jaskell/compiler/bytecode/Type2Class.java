...
import oqube.bytes.instructions.TypedInst;
...
/**
 * A class offering methods for generating java classes from haskell type information This class offers several methods
 * for generating java class objects from haskell type information, either type expressions or type definitions.
 *
 * @author  bailly
...
public class Type2Class extends BytecodeGenerator implements TypeVisitor {
    ...
    ByteArrayClassLoader temploader = new ByteArrayClassLoader(this.getClass().getClassLoader());
     ...
     *
     * @param context a Set containing instances of TypeConstraint
     */
    ...
    /**
     * returns a sequence of instruction to encapsulate an object of type type in an Object reference. The data to
     * encapsulate is assumed to lay on top of the stack Needs a ClassFile to generate constant entries
     */
    public static Instruction box(ClassFile cf, Class c) {
        ...
        if (c.isPrimitive()) {
            ...
            case 'I':
                d = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JInt");
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JInt", "<init>", "(I)V");
                seq._new(d)._dup_x1()._swap()._invokespecial(m, cf);
                return seq;

            // we've got an initialized integer on the stack
            case 'Z':
                d = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JBoolean");
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JBoolean", "<init>", "(Z)V");
                seq._new(d)._dup_x1()._swap()._invokespecial(m, cf);
                return seq;

            case 'F':
                d = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JFloat");
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JFloat", "<init>", "(F)V");
                seq._new(d)._dup_x1()._swap()._invokespecial(m, cf);
                return seq;

            case 'D':
                d = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JDouble");
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JDouble", "<init>", "(D)V");
            ...
            case 'C':
                d = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JChar");
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JChar", "<init>", "(C)V");
                seq._new(d)._dup_x1()._swap()._invokespecial(m, cf);
                return seq;
    ...
    /**
     * returns a sequence of instruction to decapsulate a JObject to the type given by c. There is a problem only for
     * primitive data types. The data to encapsulate is assumed to lay on top of the stack Needs a ClassFile to generate
     * constant entries
     */
    public static Instruction unbox(ClassFile cf, Class c) {
        Sequence seq = new Sequence(cf);
        short d, m, cli;
        cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue");
        if (c.isPrimitive()) {
            switch (TypeHelper.getInternalName(c).charAt(0)) {

            case 'I':
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "asInt", "()I");
                seq._checkcast(cli)._invokevirtual(m, cf); // unpakc
                return seq;

            // we've got an initialized integer on the stack
            case 'Z':
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "asBool", "()Z");
                seq._checkcast(cli)._invokevirtual(m, cf); // unpakc
                return seq;

            case 'F':
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "asFloat", "()F");
                seq._checkcast(cli)._invokevirtual(m, cf); // unpakc
                return seq;

            case 'D':
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "asDouble", "()D");
                seq._checkcast(cli)._invokevirtual(m, cf); // unpakc
                return seq;
            ...
            case 'C':
                m = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "asChar", "()C");
                seq._checkcast(cli)._invokevirtual(m, cf); // unpakc
                return seq;
            ...
            cli = ClassData.create(cf.getConstantPool(), c.getName().replace('.', '/'));
    ...
    /**
     * Method encodeType2Bytecode. Returns the signature of the given type as encoded in class files.
     *
     * @param  type
     *
     * @return String
    ...
    public static String encodeType2Bytecode(Type type) {
        ...
        if (type instanceof TypeApplication) { // iterate over function args
            ...
            while (fti.hasNext()) {
                cur = (Type) fti.next();
                tsig.append(((TypeApplication) ((TypeApplication) cur).getDomain()).getRange().visit(tv));
            }
            tsig.append(")");
            // append return type
    ...
    /**
     * Method cast. This method returns checkcast instruction according to wrapper class of class c
     *
     * @param  c
     *
     * @return Instruction
    ...
    public Instruction cast(ClassFile cf, Class c) {
        ...
        if (c.isPrimitive()) {
            ...
            case 'I':
                cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JInt");
                break;

            // we've got an initialized integer on the stack
            case 'Z':
                cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JBoolean");
                break;

            case 'F':
                cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JFloat");
                break;

            case 'D':
                cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JDouble");
                break;
            ...
            case 'C':
                cli = ClassData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JChar");
                break;
    ...
    public Object visit(TypeConstructor t) {
        String dname;
        if (module != null)
            dname = BytecodeGenerator.encodeName2Java(((Definition) module.resolveType(t.toString())).getName());
        else {
            /*
    ...
    public Object visit(ConstructorDefinition cdef) {
        ...
        try {
            ...
            try {
                ...
                return generateClassFromCtor("fr/lifl/jaskell/runtime/types/JMessage", null, cdef);
            }
        }
    }
    ...
    public Object visit(Module a) {
        ...
        while (it.hasNext()) {
            ...
            try {
                log.finest("In module " + a.getName() + ", visiting symbol " + def.getName());
    ...
    private Class generateClassFromCtor(String dataname, ClassFile datafile, ConstructorDefinition type) {
        /* compute class name */
        ...
        short supref = MethodRefData.create(cf.getConstantPool(), dataname, "<init>", supersig);
        ...
        if (l.size() == 0) {
            ...
            ffi.setFlags((short) (Constants.ACC_PUBLIC | Constants.ACC_FINAL | Constants.ACC_STATIC));
            ...
            short fref = FieldRefData.create(cf.getConstantPool(), tname, "_instance", "L" + dataname + ";");
            supref = MethodRefData.create(cf.getConstantPool(), tname, "<init>", initsig.toString());
            seq._new(clref)._dup()._invokespecial(supref, cf)._putstatic(fref, cf)._return();
            code.add(seq);
            mfi.addAttribute(code);
            cf.add(mfi);
        } else {
    ...
    private Object generateClassFromData(DataDefinition ddef) {
        ...
        mfi.setName("<init>");
        mfi.setType("()V");
        short supref = MethodRefData.create(cf.getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", "<init>", "()V");
