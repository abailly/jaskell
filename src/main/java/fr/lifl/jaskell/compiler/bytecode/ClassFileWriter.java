...
import fr.norsys.klass.bytes.LineNumberTableAttribute;
import fr.norsys.klass.bytes.MethodFileInfo;
import fr.norsys.klass.bytes.SourceFileAttribute;

/**
 * Writes class files to disk
 *
 * This class writes the ClassFile objects to .class files. The
 * package fr.lifl.directories are created if non existent, relative to 
 * a root directory.
 */
public class ClassFileWriter implements ClassFileHandler {
	...
	/**
	 * Parameter less constructor. Files are written relative
	...
	/**
	 * Constructor which sets the root directory from where files
	 * are created
	 *
	...
	/**
	 * Generate a source for given classfile which represents code in
	 * assembly form. This method adds a SourceFile and LineAttribute
	 * attributes to the given classfile. 
	 * 
