...
/**
 * A visitor to generate tests for patterns This Visitor class generates instructions sequences to assert that a given
 * value matches a given pattern. This instruction sequences should leave on the stack a value which is non zero if
 * pattern matches and zero otherwise.
 *
 * @author  bailly
...
public class PatternCodeGenerator extends JaskellVisitorAdapter {
    ...
    public Object visit(CharLiteral a) {
        Sequence seq = new Sequence(classFile);
        LocalBinding e = ((Alternative) a.getParent()).getBinding();
        seq.add((Instruction) codegen.generateLoad(e)).add(codegen.generateConstantInt(a.getChar()));
        seq._if_icmpne(0);
        return seq;
    }
    ...
    public Object visit(ConstructorPattern a) {
        ...
        /* retrieve definition */
        ConstructorDefinition cdef = (ConstructorDefinition) ctor.lookup(ctor.getName());
        Type2Class tc = new Type2Class(codegen.getNamespace());
        Class dcls = (Class) cdef.visit(tc);
        /* generate test for class */
        short cref = ClassData.create(codegen.getClassFile().getConstantPool(), dcls.getName().replace('.', '/'));
        ...
        while (it.hasNext()) {
            ...
            if (cdef.isStrict(i))
                fref = FieldRefData.create(codegen.getClassFile().getConstantPool(), dcls.getName().replace('.', '/'), "_" + i, psig);
            else
                fref = FieldRefData.create(codegen.getClassFile().getConstantPool(), dcls.getName().replace('.', '/'), "_" + i, "Lfr/lifl/jaskell/runtime/types/JObject;");

            /* extract parameter from ctor */
            Sequence pseq = new Sequence(classFile).add((Instruction) codegen.generateLoad(e))._checkcast(cref)._getfield(fref, codegen.getClassFile());
            /* call eval if parameter is not strict */
            if (!cdef.isStrict(i)) {
                short evalref = InterfaceMethodData.create(codegen.getClassFile().getConstantPool(), "fr/lifl/jaskell/runtime/types/JObject", "eval", "()Lfr/lifl/jaskell/runtime/types/JObject;");
                ...
                if (cls.isPrimitive() || cls.equals(java.lang.String.class)) {
                    ...
                    short evref = MethodRefData.create(codegen.getClassFile().getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue", ev, "()" + psig);
                    short clref = ClassData.create(codegen.getClassFile().getConstantPool(), "fr/lifl/jaskell/runtime/types/JValue");
    ...
    public Object visit(FloatLiteral a) {
        Sequence seq = new Sequence(classFile);
        LocalBinding e = ((Alternative) a.getParent()).getBinding();
        seq.add((Instruction) codegen.generateLoad(e)).add(codegen.generateConstantFloat(a.getFloat(), classFile))._fcmpg();
    ...
    public Object visit(IntegerLiteral a) {
        Sequence seq = new Sequence(classFile);
        LocalBinding e = ((Alternative) a.getParent()).getBinding();
        seq.add((Instruction) codegen.generateLoad(e)).add(codegen.generateConstantInt(a.getInteger()));
    ...
    public Object visit(StringLiteral a) {
        ...
        short sref = StringData.create(codegen.getClassFile().getConstantPool(), a.getString());
        /* reference for equals() method */
        short eqref = MethodRefData.create(codegen.getClassFile().getConstantPool(), "java/lang/Object", "equals", "(Ljava/lang/Object;)Z");
        Sequence seq = new Sequence(classFile);
        LocalBinding e = ((Alternative) a.getParent()).getBinding();
        seq.add((Instruction) codegen.generateLoad(e));
