...
import fr.lifl.jaskell.compiler.types.TypeInstantiator;
import fr.lifl.jaskell.compiler.types.TypeSubstitution;
import fr.lifl.jaskell.compiler.types.TypeUnifier;
...
/**
 * A type inference engine for jaskell expressions
 * 
 * This visitor tries to verify that typing information is correct
 * for a set of expressions, or that it can be inferred from subexpressions.<p>
 * The visitor recursively checks sub-expressions for type of leaves and then tries
 * to unify leaf types with node types. If unification is possible, then the
 * super-expression type is set and returned.
 * 
 * @author bailly
...
public class TypeChecker extends CompilerPass {
	...
	public Object visit(Abstraction a) {
		try {
			...
			Type ret =
				tu.unify(PrimitiveType.getReturnType(ft), bt, typeVariablesMap);
			TyvarSubstitution tys = new TyvarSubstitution(typeVariablesMap);
			tys.visit(a);
			log.finer(
				"Done abstraction, setting type from "
					+ ft
					+ " to "
					+ a.getType());
			popContext();
	...
	/**
	 * Save a stack of names and type vars maps. Given argument
	 * is list of bindings from which we construct a linked map to
	 * store names
	 * 
	 * @param linkedHashMap
	 */
	...
	public Object visit(Alternative a) {
		try {
			...
			while (it.hasNext()) {
				...
				log.finer(
					"In alternative, unifying pattern type "
						+ pt
						+ " to "
						+ ptype);
				...
				log.finer(
					"In alternative, unifying body type "
						+ bt
						+ " to "
						+ btype);
			}
	...
	public Object visit(Application a) {
		try {
			...
			log.finer(
				"In application, trying to unify function type "
					+ t
					+ " with body "
					+ ft);
			/* 
			 ...
			 */
			TypeApplication uni =
				(TypeApplication) tu.unify(t, ft, typeVariablesMap);
			...
			while (it2.hasNext()) {
				...
				/* try unification */
				tu.unify(
					((TypeApplication) ut.getDomain()).getRange(),
					at,
					new HashMap(typeVariablesMap));
			}
			ft = subst.substitute(ft);
			fun.setType(ft);
			log.finer(
				"Setting type of functional element [" + fun + "] to :" + ft);
	...
	/**
	 * Returns null or throws a TypeError if type cannot be inferred
	 * for any definition found. Verifies that main symbol in Main module
	 * is typed as <code>IO ()</code>.
	 * 
	...
	public Object visit(ConstructorPattern a) {
		...
		ConstructorDefinition ctor =
			(ConstructorDefinition) a.getConstructor().lookup(cname);
		if (ctor == null)
		...
		while (it.hasNext()) {
			try {
				...
				throw new TypeError(
					"Wrong number of arguments to pattern " + a);
			}
		}
		a.setType(rtype);
		return a.getType();
	...
	public Object visit(Conditional conditional) {
		...
		if (!(tcond.equals(Primitives.BOOL)))
			throw new TypeError(
				"Conditional expression is not of type Bool : " + tcond);
	...
	public Object visit(Module a) {
		try {
			...
			if (te.getLineCol() == null)
				te.setLineCol(a.getTag("source"));
			log.severe(
				"Type error at (" + te.getLineCol() + ") : " + te.getMessage());
			throw te;
		}
	}
