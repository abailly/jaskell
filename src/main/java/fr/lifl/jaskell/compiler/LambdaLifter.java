...
/**
 * A class that implements lambda lifting
 * 
 * This class is used to implement lambda-lifting. It is used either on Module
 * object to clean-up globally all abstractions in this module or on specific
 * definitions. In the latter case, the parameterized constructor may be used to
 * define the scope where abstractions should be lifted to.
 * <p>
 * The algorithm used is simple : each abstraction which is not at top level is
 * scanned for <code>LocalBinding</code> objects which are defined in an
 * enclosing scope. This bindings are then substituted for bindings local to the
 * abstraction which lifted to the enclosing Namespace and given a name. All
 * occurences of this abstraction is then replaced by calls to the lifted
 * definition.
 * <p>
 * 
 * @author bailly
...
public class LambdaLifter extends JaskellVisitorAdapter {
  ...
  /**
   * This method produces a new Abstraction which is the result of adding
   * captured variables as new arguments and bind it in current ns. It returns
   * the name of the bound funciton.
   * 
   * @param a
   * @param captured
  ...
  /**
   * Returns an Abstraction object resulting from lifting the given expression
   * with captured variable set. if <code>f x y</code> is an expression with
   * <code>y</code> a captured variable, then it is transformed into
   * <code>lambdaXXX = \ y -> f x y</code> which can later be used to replace
   * the original expression with <code>lambdaXXX y</code>.
   * 
   * @param e
   * @param captured
  ...
  /**
   * This method produces an Expression from an arbitrary Abstraction object
   * which is applied to a list of Variables referencing captured bindings.
   * 
   * @param a
   *          an Abstraction object to partially apply
   * @param captured
   * @return
   */
  ...
  public Object visit(Conditional conditional) {
    conditional.setCondition((Expression) conditional.getCondition()
        .visit(this));
