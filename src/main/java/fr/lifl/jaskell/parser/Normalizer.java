...
/**
 * A class that transforms a list of equations into core expressions ready to be
 * sent to code generator
 * 
 * This class transforms the output of Yyparser, that is a map of definition
 * names to list of equations into a list of core definitions which are then
 * bound into the given context. Normalizer only performs syntactic
 * transformations, that is mainly :
 * <ul>
...
public class Normalizer {
   ...
   * Normalizing method.
   * 
   * This method normalizes a Map from names to list of equations.
   * 
   * @param map
   *          a Map associating names to list of equations
   */
  ...
  /**
   * A method that checks a set of equations are consistent and which builds a
   * list of bindings
   * 
   * @param eqs
   * @return List
    ...
    while (it.hasNext()) {
      ...
      if (n == -1)
        ...
        throw new IllegalStateException(
            "Inconsistent number of arguments : expected " + n + ", found "
                + narg);
    }
    /* build variable list */
